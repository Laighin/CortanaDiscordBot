...
module.exports = {
    /**
     * This command bans a given user from the server.
     * It checks for various conditions such as user roles and permissions before proceeding with the ban.
    ...
    callback: async (client, interaction) => {
        ...
        if (targetUser.id === interaction.guild.ownerId) {
            const ownerResponses = [
                "You can't ban the server owner. Are you crazy?",
        ...
        // Check if the target user has a role higher than the request user
        if (targetUserRolePosition >= requestUserRolePosition) {
            await interaction.editReply(`Sorry ${interaction.member.displayName}, but you can't ban someone with the same role as you or higher!`);
            return;
        }
        ...
        if (targetUserRolePosition >= botRolePosition) {
            const excuses = [
                ...
                "I can't ban that user... apparently, I'm not 'important' enough. Maybe I should start my own server. 🤔",
                "I can't ban them because they outrank me! Maybe if you gave me a better role, I wouldn't be so powerless. 🤷",
        ...
        try {
            ...
            await interaction.editReply(`🚨 **${targetUser}** has been banned!\n**Reason:** ${reason} 🚨`);
        } catch (error) {
            console.log(`Cortana encountered an error when banning a user: ${error}`);
        }
    },
